...
public class SlashCommandListener extends ListenerAdapter {
	...
	void stopCommand(SlashCommandInteractionEvent event) {
	...
	void newCommand(SlashCommandInteractionEvent event) {
		try {
	...
	void removeCommand(SlashCommandInteractionEvent event) {
		try {
			String suffix = event.getOption("suffix").getAsString();
			suffix = Utils.normalizeText(suffix);
	...
	void ignoreMeCommand(SlashCommandInteractionEvent event) {
		try {
	...
	void setChanceCommand(SlashCommandInteractionEvent event) {
		try {
			Optional<Options> userOptions = optionsRepo.findById(event.getUser().getId());
			if(userOptions.isEmpty()) {
				Options newOptions = new Options(
	...
	void listCommand(SlashCommandInteractionEvent event) {
		try {
			final String[] list = {""};
	...
	void helpCommand(SlashCommandInteractionEvent event) {
		event.reply("""
			rima_bot Commands:
